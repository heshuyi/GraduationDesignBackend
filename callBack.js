...
var getAuctionList = function (req, res) {
    ...
    let sql = `SELECT sellgoodstable.endtime,sellgoodstable.goodsid,sellgoodstable.goodsname,sellgoodstable.imgflie,sellgoodstable.money,sellgoodstable.tel,sellgoodstable.text
             ...
             WHERE shoppingcar.tel=? AND shoppingcar.goodsid = sellgoodstable.goodsid ORDER BY shoppingcar.goodsid`
    let sqlArr = [tel]
    dbConfig.sqlConnect(sql, sqlArr, (err, data) => {
        if (err) {
            console.log(err)
...
let getMyGoodsMoney = function (req, res) {
    let {tel} = req.body
    let sql = 'select * from newestgoodsmoney where newTel = ? ORDER BY newGoodid '
    let sqlArr = [tel]
    dbConfig.sqlConnect(sql, sqlArr, (err, data) => {
        if (err) {
            res.send({
...
let getAllGoodsMoney = function (req, res) {
